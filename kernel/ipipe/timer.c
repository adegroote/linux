...
//XXX duplicate from time/timekeeping
...
void ipipe_update_hostrt(struct timekeeper *tk)
{
	...
	ipipe_root_only();
	data.live = 1;
	data.cycle_last = tk->tkr.cycle_last;
	data.mask = tk->tkr.clock->mask;
	data.mult = tk->tkr.mult;
	data.shift = tk->tkr.shift;
	data.wall_time_sec = xt.tv_sec;
	data.wall_time_nsec = xt.tv_nsec;
	data.wall_to_monotonic = timespec64_to_timespec(tk->wall_to_monotonic);
